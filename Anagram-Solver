...
//Student: Manuel Gomes Rosmaninho
...
void FindAnagrams(string originalString) { //this function is designed to expect one argument (called originalString)
  ...
  while(!dbFile.eof()) { //this sets up a loop that repeats until we have read every line in the opened file; line by line
    ...
    currentWord.erase(remove_if(currentWord.begin(), currentWord.end(), [](unsigned char x){return isspace(x);}), currentWord.end()); //this line of code removes all spaces from a string 'currentWord' by shifting all non-space characters to the left and then erasing the extra
